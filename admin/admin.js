  ...
  function addLine(){
    ...
    div.innerHTML=`
      <div class="purchase-mode improved-mode">
        ...
        <p class="help-text">Choose an existing fragrance from the list to restock it, or switch to New bottle to add something new to the Catalogue.</p>
      </div>
    ...
    function setMode(){
      ...
      if(isNew){
        ...
        if(newName && !newName.value && existingName.value) newName.value=existingName.value.replace(/\s+·\s+.*$/,'');
      }
    }
    action.addEventListener('change',setMode); setMode();
    function checkExisting(){
      const match=findExisting(existingName.value);
      if(match){
        existingId.value=match.id;
        status.textContent=`Matched existing bottle: ${match.name} · ${match.house}. This will be saved as a restock/update.`;
        status.classList.add('good');
      } else {
        existingId.value='';
        status.textContent='No exact match yet. Select a fragrance from the list, or change Bottle action to New bottle.';
